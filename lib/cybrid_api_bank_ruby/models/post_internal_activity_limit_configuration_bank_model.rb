...
module CybridApiBank
  ...
  class PostInternalActivityLimitConfigurationBankModel
    # The type of the configuration.
    attr_accessor :type

    # The activity type of the configuration.
    ...
    # The audience for the configuration. Required when type is platform or type is bank.
    attr_accessor :audience

    # The country code for the configuration. Required when type is platform.
    attr_accessor :country_code

    # The bank GUID for the configuration. Required when type is bank.
    attr_accessor :bank_guid

    # The customer GUID for the configuration. Required when type is customer.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalActivityLimitConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalActivityLimitConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["platform", "bank", "customer"])
      return false unless type_validator.valid?(@type)
      activity_validator = EnumAttributeValidator.new('String', ["trading", "funding", "book_transfer", "crypto_transfer"])
      ...
      audience_validator = EnumAttributeValidator.new('String', ["individual_tax_id_unverified_customer", "individual_tax_id_verified_customer", "business_customer"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["platform", "bank", "customer"])
    ...
    def activity=(activity)
      validator = EnumAttributeValidator.new('String', ["trading", "funding", "book_transfer", "crypto_transfer"])
    ...
    def audience=(audience)
      validator = EnumAttributeValidator.new('String', ["individual_tax_id_unverified_customer", "individual_tax_id_verified_customer", "business_customer"])
    ...
    def hash
      [type, activity, side, limits, environment, audience, country_code, bank_guid, customer_guid].hash
    end
