  ...
  class HoldDetailBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::HoldDetailBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::HoldDetailBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'eta_at')
        self.eta_at = attributes[:'eta_at']
      end
    end
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["customer_hold_duration", "reserve_below_minimum", "screening_administrative", "screening_non_administrative"])
      return false unless type_validator.valid?(@type)
      return false if @state.nil?
      state_validator = EnumAttributeValidator.new('String', ["active", "completed", "storing"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["customer_hold_duration", "reserve_below_minimum", "screening_administrative", "screening_non_administrative"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["active", "completed", "storing"])
    ...
    def hash
      [type, state, started_at, completed_at, eta_at].hash
    end
