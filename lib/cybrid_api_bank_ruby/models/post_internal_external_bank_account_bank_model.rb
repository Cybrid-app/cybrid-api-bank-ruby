...
module CybridApiBank
  class PostInternalExternalBankAccountBankModel
    ...
    # The counterparty's email address on the account (internal).
    ...
    def self.openapi_types
      {
        ...
        :'counterparty_bank_account' => :'PostInternalExternalBankAccountCounterpartyBankAccountBankModel',
        :'counterparty_name' => :'PostInternalExternalBankAccountCounterpartyNameBankModel',
        :'counterparty_address' => :'PostInternalExternalBankAccountCounterpartyAddressBankModel',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalExternalBankAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalExternalBankAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@beneficiary_memo.nil? && @beneficiary_memo.to_s.length > 128
        invalid_properties.push('invalid value for "beneficiary_memo", the character length must be smaller than or equal to 128.')
      end

      if !@beneficiary_memo.nil? && @beneficiary_memo.to_s.length < 1
        invalid_properties.push('invalid value for "beneficiary_memo", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      account_kind_validator = EnumAttributeValidator.new('String', ["modern_treasury_external_account", "plaid", "plaid_processor_token", "raw_routing_details"])
    ...
    def account_kind=(account_kind)
      validator = EnumAttributeValidator.new('String', ["modern_treasury_external_account", "plaid", "plaid_processor_token", "raw_routing_details"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] beneficiary_memo Value to be assigned
    def beneficiary_memo=(beneficiary_memo)
      if !beneficiary_memo.nil? && beneficiary_memo.to_s.length > 128
        fail ArgumentError, 'invalid value for "beneficiary_memo", the character length must be smaller than or equal to 128.'
      end
    ...
    def hash
      [name, account_kind, provider_id, beneficiary_memo, exchange_guid, bank_guid, customer_guid, asset, plaid_public_token, plaid_account_id, counterparty_bank_account, counterparty_name, counterparty_address, counterparty_email_address].hash
    end
