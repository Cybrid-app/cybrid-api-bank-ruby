...
module CybridApiBank
  class InternalActivityLimitConfigurationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalActivityLimitConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalActivityLimitConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@customer_guid.nil? && @customer_guid.to_s.length < 32
      audience_validator = EnumAttributeValidator.new('String', ["individual_tax_id_verified_customer", "individual_tax_id_unverified_customer", "business_customer"])
    ...
    def audience=(audience)
      validator = EnumAttributeValidator.new('String', ["individual_tax_id_verified_customer", "individual_tax_id_unverified_customer", "business_customer"])
    ...
    def hash
      [guid, type, activity, side, environment, bank_guid, customer_guid, audience, country_code, limits, created_at, updated_at].hash
    end
