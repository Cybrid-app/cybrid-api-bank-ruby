...
module CybridApiBank
  ...
  class PostPlanBankModel
    ...
    attr_accessor :intermediate_accounts

    attr_accessor :travel_rule_info

    # The purpose of transaction for the plan.
    attr_accessor :purpose_of_transaction
    ...
    def self.openapi_types
      {
        ...
        :'intermediate_accounts' => :'Array<PostPlanIntermediateAccountsInnerBankModel>',
        :'travel_rule_info' => :'PostPlanTravelRuleInfoBankModel',
        :'purpose_of_transaction' => :'String',
        :'labels' => :'Array<String>',
        :'supporting_documents' => :'Array<PostSupportingDocumentBankModel>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostPlanBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostPlanBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'source_account')
        ...
        self.source_account = nil
      end

      if attributes.key?(:'destination_account')
        self.destination_account = attributes[:'destination_account']
      else
        self.destination_account = nil
      end

      if attributes.key?(:'intermediate_accounts')
        if (value = attributes[:'intermediate_accounts']).is_a?(Array)
          self.intermediate_accounts = value
        end
      end
      ...
      if attributes.key?(:'supporting_documents')
        if (value = attributes[:'supporting_documents']).is_a?(Array)
          self.supporting_documents = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if !@supporting_documents.nil? && @supporting_documents.length > 3
        invalid_properties.push('invalid value for "supporting_documents", number of items must be less than or equal to 3.')
      end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["remittance", "bill_pay", "disbursement", "invoice_pay", "deposit", "withdrawal", "conversion"])
      ...
      return false if @destination_account.nil?
      ...
      return false if !@supporting_documents.nil? && @supporting_documents.length > 3
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["remittance", "bill_pay", "disbursement", "invoice_pay", "deposit", "withdrawal", "conversion"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] purpose_of_transaction Object to be assigned
    def purpose_of_transaction=(purpose_of_transaction)
      validator = EnumAttributeValidator.new('String', ["computer_services", "family_support", "education", "gift", "charitable_donation", "medical_treatment", "maintenance_expenses", "travel", "hotel_accommodation", "small_value_remittance", "liberalized_remittance", "personal_transfer", "loan_payment", "tax_payment", "construction_expenses", "advertising_expenses", "advisory_fees", "business_insurance", "insurance_claims", "delivery_fees", "service_charges", "office_expenses", "property_purchase", "property_rental", "royalty_fees", "shares_investment", "fund_investment", "bill_payment", "transportation_fees", "salary_payment", "reward_payment", "influencer_payment", "other_fees", "other", "exported_goods", "commission_on_goods", "commission_on_services", "accounting_services", "exhibition_services"])
      unless validator.valid?(purpose_of_transaction)
        fail ArgumentError, "invalid value for \"purpose_of_transaction\", must be one of #{validator.allowable_values}."
      end
      @purpose_of_transaction = purpose_of_transaction
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] supporting_documents Value to be assigned
    def supporting_documents=(supporting_documents)
      if !supporting_documents.nil? && supporting_documents.length > 3
        fail ArgumentError, 'invalid value for "supporting_documents", number of items must be less than or equal to 3.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          intermediate_accounts == o.intermediate_accounts &&
    ...
    def hash
      [type, bank_guid, customer_guid, source_account, destination_account, intermediate_accounts, travel_rule_info, purpose_of_transaction, labels, supporting_documents].hash
    end
