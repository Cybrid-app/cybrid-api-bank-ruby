  ...
  class PaymentInstructionBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PaymentInstructionBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PaymentInstructionBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@invoice_guid.nil? && @invoice_guid.to_s.length > 32
    ...
    def valid?
      ...
      return false if !@customer_guid.nil? && @customer_guid.to_s.length < 32
      return false if !@invoice_guid.nil? && @invoice_guid.to_s.length > 32
      return false if !@invoice_guid.nil? && @invoice_guid.to_s.length < 32
      true
    end
    ...
    def hash
      [guid, customer_guid, invoice_guid, created_at, updated_at, expired_at, failed_at, network_address, expected_payment_asset, expected_payment_amount, failure_code, state].hash
    end
