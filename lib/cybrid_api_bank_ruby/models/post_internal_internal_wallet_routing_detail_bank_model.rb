...
module CybridApiBank
  class PostInternalInternalWalletRoutingDetailBankModel
    attr_accessor :route_type

    # The provider supplied identifier for the route
    attr_accessor :provider_id

    # The wallet service guid.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalInternalWalletRoutingDetailBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalInternalWalletRoutingDetailBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @wallet_service_guid.nil?
        invalid_properties.push('invalid value for "wallet_service_guid", wallet_service_guid cannot be nil.')
      end

      if @wallet_service_guid.to_s.length > 32
    ...
    def valid?
      ...
      route_type_validator = EnumAttributeValidator.new('String', ["fireblocks_external_wallet", "circle_recipient_address", "strike_recipient_address", "bitgo_recipient_address", "falconx_whitelisted_address", "aquanow_recipient_address", "bitso_destination_address"])
      return false unless route_type_validator.valid?(@route_type)
      return false if @provider_id.nil?
      return false if @wallet_service_guid.nil?
      return false if @wallet_service_guid.to_s.length > 32
      return false if @wallet_service_guid.to_s.length < 32
      true
    end
    ...
    def route_type=(route_type)
      validator = EnumAttributeValidator.new('String', ["fireblocks_external_wallet", "circle_recipient_address", "strike_recipient_address", "bitgo_recipient_address", "falconx_whitelisted_address", "aquanow_recipient_address", "bitso_destination_address"])
    ...
    def wallet_service_guid=(wallet_service_guid)
      if wallet_service_guid.nil?
        fail ArgumentError, 'wallet_service_guid cannot be nil'
      end
    ...
    def hash
      [route_type, provider_id, wallet_service_guid].hash
    end
