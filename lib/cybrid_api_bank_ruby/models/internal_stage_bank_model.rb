  ...
  class InternalStageBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalStageBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalStageBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'stage_index')
        self.stage_index = attributes[:'stage_index']
      end
    ...
    def list_invalid_properties
      ...
      if @stage_index.nil?
        invalid_properties.push('invalid value for "stage_index", stage_index cannot be nil.')
      end

      if @stage_index < 0
        invalid_properties.push('invalid value for "stage_index", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @plan_guid.to_s.length < 32
      return false if @stage_index.nil?
      return false if @stage_index < 0
    ...
    # Custom attribute writer method with validation
    # @param [Object] stage_index Value to be assigned
    def stage_index=(stage_index)
      if stage_index.nil?
        fail ArgumentError, 'stage_index cannot be nil'
      end

      if stage_index < 0
        fail ArgumentError, 'invalid value for "stage_index", must be greater than or equal to 0.'
      end
    ...
    def hash
      [guid, plan_guid, stage_index, type, state, failure_code, created_at, updated_at, source_account, destination_account, fees].hash
    end
