...
module CybridApiBank
  class InternalCryptoAssetConfigurationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalCryptoAssetConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalCryptoAssetConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@maximum_number_of_addresses.nil? && @maximum_number_of_addresses < 0
        invalid_properties.push('invalid value for "maximum_number_of_addresses", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@bank_guid.nil? && @bank_guid.to_s.length < 32
      return false if !@maximum_number_of_addresses.nil? && @maximum_number_of_addresses < 0
    ...
    def maximum_number_of_addresses=(maximum_number_of_addresses)
      if !maximum_number_of_addresses.nil? && maximum_number_of_addresses < 0
        fail ArgumentError, 'invalid value for "maximum_number_of_addresses", must be greater than or equal to 0.'
      end
    ...
    def hash
      [guid, type, environment, bank_guid, deposits_enabled, invoices_enabled, storage_enabled, maximum_number_of_addresses, asset, created_at, updated_at].hash
    end
