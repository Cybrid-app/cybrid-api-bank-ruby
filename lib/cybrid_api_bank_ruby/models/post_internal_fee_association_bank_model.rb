...
module CybridApiBank
  ...
  class PostInternalFeeAssociationBankModel
    # The type of fee.
    attr_accessor :type

    # The asset code associated with the stage.
    attr_accessor :asset

    # The unique identifier for the step.
    attr_accessor :step_identifier

    # The estimated fee amount in base units of the currency.
    attr_accessor :quoted_amount

    # The executed fee amount in base units of the currency.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalFeeAssociationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalFeeAssociationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [type, asset, step_identifier, quoted_amount, executed_amount].hash
    end
