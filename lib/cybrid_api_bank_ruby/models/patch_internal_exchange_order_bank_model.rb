...
module CybridApiBank
  ...
  class PatchInternalExchangeOrderBankModel
    # The unique identifier for the order in the provider system.
    attr_accessor :provider_order_guid
    ...
    # @param [Object] provider_order_guid Value to be assigned
    def provider_order_guid=(provider_order_guid)
      if !provider_order_guid.nil? && provider_order_guid.to_s.length > 64
        fail ArgumentError, 'invalid value for "provider_order_guid", the character length must be smaller than or equal to 64.'
      end
