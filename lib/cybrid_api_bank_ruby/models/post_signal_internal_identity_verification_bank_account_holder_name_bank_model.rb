...
module CybridApiBank
  ...
  class PostSignalInternalIdentityVerificationBankAccountHolderNameBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostSignalInternalIdentityVerificationBankAccountHolderNameBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostSignalInternalIdentityVerificationBankAccountHolderNameBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @full.to_s.length > 128
        invalid_properties.push('invalid value for "full", the character length must be smaller than or equal to 128.')
      end

      if @full.to_s.length < 1
        invalid_properties.push('invalid value for "full", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @full.to_s.length > 128
      return false if @full.to_s.length < 1
      true
    end

    # Custom attribute writer method with validation
    # @param [Object] full Value to be assigned
    def full=(full)
      if full.nil?
        fail ArgumentError, 'full cannot be nil'
      end

      if full.to_s.length > 128
        fail ArgumentError, 'invalid value for "full", the character length must be smaller than or equal to 128.'
      end

      if full.to_s.length < 1
        fail ArgumentError, 'invalid value for "full", the character length must be great than or equal to 1.'
      end
