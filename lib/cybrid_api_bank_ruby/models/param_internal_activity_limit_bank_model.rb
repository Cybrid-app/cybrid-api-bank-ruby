...
module CybridApiBank
  ...
  class ParamInternalActivityLimitBankModel
    ...
    # The interval of the limit. Required when limit_type is rolling.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::ParamInternalActivityLimitBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::ParamInternalActivityLimitBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @limit_asset.to_s.length > 16
        invalid_properties.push('invalid value for "limit_asset", the character length must be smaller than or equal to 16.')
      end

      if @limit_asset.to_s.length < 1
        invalid_properties.push('invalid value for "limit_asset", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      limit_type_validator = EnumAttributeValidator.new('String', ["rolling", "daily", "weekly", "monthly"])
      return false unless limit_type_validator.valid?(@limit_type)
      return false if @limit_asset.nil?
      return false if @limit_asset.to_s.length > 16
      return false if @limit_asset.to_s.length < 1
    ...
    def limit_type=(limit_type)
      validator = EnumAttributeValidator.new('String', ["rolling", "daily", "weekly", "monthly"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] limit_asset Value to be assigned
    def limit_asset=(limit_asset)
      if limit_asset.nil?
        fail ArgumentError, 'limit_asset cannot be nil'
      end

      if limit_asset.to_s.length > 16
        fail ArgumentError, 'invalid value for "limit_asset", the character length must be smaller than or equal to 16.'
      end

      if limit_asset.to_s.length < 1
        fail ArgumentError, 'invalid value for "limit_asset", the character length must be great than or equal to 1.'
      end
