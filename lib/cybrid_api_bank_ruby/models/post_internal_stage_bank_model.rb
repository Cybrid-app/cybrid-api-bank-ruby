...
module CybridApiBank
  ...
  class PostInternalStageBankModel
    # The type of stage.
    attr_accessor :type
    ...
    # The timestamp when the stage is planned, if applicable.
    ...
    # The index of the stage within the plan, starting from 0.
    ...
    def self.openapi_types
      {
        ...
        :'failed_at' => :'Time',
        :'planned_at' => :'Time',
        :'accounts' => :'Array<PostInternalAccountAssociationBankModel>',
        :'fees' => :'Array<PostInternalFeeAssociationBankModel>',
        :'stage_index' => :'Integer'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalStageBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalStageBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'planned_at')
        self.planned_at = attributes[:'planned_at']
      end
    ...
    def hash
      [type, plan_guid, failure_code, failed_at, planned_at, accounts, fees, stage_index].hash
    end
