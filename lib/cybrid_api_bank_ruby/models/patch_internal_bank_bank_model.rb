...
module CybridApiBank
  class PatchInternalBankBankModel
    # True if the bank supports raw routing details for external bank accounts.
    attr_accessor :raw_routing_details

    # True if the bank counterparty external bank accounts.
    attr_accessor :counterparty_external_accounts
    ...
    def self.openapi_types
      {
        :'raw_routing_details' => :'Boolean',
        :'counterparty_external_accounts' => :'Boolean',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalBankBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalBankBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'raw_routing_details')
        self.raw_routing_details = attributes[:'raw_routing_details']
      end

      if attributes.key?(:'counterparty_external_accounts')
        self.counterparty_external_accounts = attributes[:'counterparty_external_accounts']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          raw_routing_details == o.raw_routing_details &&
          counterparty_external_accounts == o.counterparty_external_accounts &&
    ...
    def hash
      [raw_routing_details, counterparty_external_accounts, persona_theme_id].hash
    end
