...
module CybridApiBank
  class InternalInternalBankAccountConfigurationBankModel
    ...
    # The identifier of the internal bank account to use for creating deposit accounts.
    attr_accessor :deposit_bank_account_guid

    # The identifier of the internal bank account to use for creating invoice accounts.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalInternalBankAccountConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalInternalBankAccountConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@asset.nil? && @asset.to_s.length > 16
    ...
    def valid?
      ...
      return false if !@bank_guid.nil? && @bank_guid.to_s.length < 32
      return false if !@asset.nil? && @asset.to_s.length > 16
    ...
    def hash
      [guid, type, environment, bank_guid, transfer_bank_account_guid, deposit_bank_account_guid, invoice_bank_account_guid, asset, created_at, updated_at].hash
    end
