...
module CybridApiBank
  class InternalExpectedPaymentBankModel
    ...
    attr_accessor :settlement_account_guid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalExpectedPaymentBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalExpectedPaymentBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'settlement_account_guid')
        self.settlement_account_guid = attributes[:'settlement_account_guid']
      end
    ...
    # @param [Object] payment_kind Object to be assigned
    def payment_kind=(payment_kind)
      validator = EnumAttributeValidator.new('String', ["gas_payment", "gas_settlement", "platform_payment", "exchange_settlement"])
      unless validator.valid?(payment_kind)
        fail ArgumentError, "invalid value for \"payment_kind\", must be one of #{validator.allowable_values}."
      end
      @payment_kind = payment_kind
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] settlement_account_guid Value to be assigned
    def settlement_account_guid=(settlement_account_guid)
      if !settlement_account_guid.nil? && settlement_account_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "settlement_account_guid", the character length must be smaller than or equal to 32.'
      end

      if !settlement_account_guid.nil? && settlement_account_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "settlement_account_guid", the character length must be great than or equal to 32.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          settlement_account_guid == o.settlement_account_guid &&
          nonce == o.nonce &&
    ...
    def hash
      [guid, payment_kind, environment, exchange_settlement_obligation_guid, credit_account_guid, settlement_account_guid, nonce, payment_amount, internal_account_guid, internal_account_type, state, created_at, updated_at].hash
    end
