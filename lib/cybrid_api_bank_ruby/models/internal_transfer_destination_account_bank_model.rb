...
module CybridApiBank
  ...
  class InternalTransferDestinationAccountBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalTransferDestinationAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalTransferDestinationAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      type_validator = EnumAttributeValidator.new('String', ["trading", "fiat", "external_bank_account", "external_wallet", "one_time_address", "reserve", "internal_wallet", "internal_bank_account", "invoice_operations", "fee", "gas", "storage"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["trading", "fiat", "external_bank_account", "external_wallet", "one_time_address", "reserve", "internal_wallet", "internal_bank_account", "invoice_operations", "fee", "gas", "storage"])
    ...
    def hash
      [guid, type, bank_guid, customer_guid].hash
    end
