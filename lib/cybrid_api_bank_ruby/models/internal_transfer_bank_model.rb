...
module CybridApiBank
  class InternalTransferBankModel
    ...
    # ISO8601 datetime the transfer was returned at.
    ...
    # ISO8601 datetime the transfer was created at.
    ...
    def self.openapi_types
      {
        ...
        :'txn_hash' => :'String',
        :'source_account' => :'InternalTransferSourceAccountBankModel',
        :'source_participants' => :'Array<TransferParticipantBankModel>',
        :'destination_account' => :'InternalTransferDestinationAccountBankModel',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalTransferBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalTransferBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@network_fee_asset.nil? && @network_fee_asset.to_s.length > 16
        invalid_properties.push('invalid value for "network_fee_asset", the character length must be smaller than or equal to 16.')
      end
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      transfer_type_validator = EnumAttributeValidator.new('String', ["funding", "book", "crypto", "internal_bank_rebalance", "internal_wallet_rebalance", "instant_funding", "funding_return", "crypto_return", "loss_recovery", "inter_account", "lightning", "instant_funding_return"])
      ...
      state_validator = EnumAttributeValidator.new('String', ["storing", "pending", "reviewing", "completed", "failed", "holding"])
      return false unless state_validator.valid?(@state)
      return false if !@reference_transfer_guid.nil? && @reference_transfer_guid.to_s.length > 32
      return false if !@reference_transfer_guid.nil? && @reference_transfer_guid.to_s.length < 32
      return false if !@network_fee_asset.nil? && @network_fee_asset.to_s.length > 16
    ...
    def transfer_type=(transfer_type)
      validator = EnumAttributeValidator.new('String', ["funding", "book", "crypto", "internal_bank_rebalance", "internal_wallet_rebalance", "instant_funding", "funding_return", "crypto_return", "loss_recovery", "inter_account", "lightning", "instant_funding_return"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "pending", "reviewing", "completed", "failed", "holding"])
    ...
    def network_fee_asset=(network_fee_asset)
      if !network_fee_asset.nil? && network_fee_asset.to_s.length > 16
        fail ArgumentError, 'invalid value for "network_fee_asset", the character length must be smaller than or equal to 16.'
      end
    ...
    def hash
      [guid, transfer_type, bank_guid, customer_guid, quote_guid, external_bank_account_guid, asset, side, state, failure_code, return_code, returned_at, reference_transfer_guid, amount, estimated_amount, fee, estimated_network_fee, network_fee, network_fee_asset, network_fee_liability_amount, network_fee_liability_amount_asset, txn_hash, source_account, source_participants, destination_account, destination_participants, created_at, updated_at, hold_started_at, hold_duration].hash
    end
