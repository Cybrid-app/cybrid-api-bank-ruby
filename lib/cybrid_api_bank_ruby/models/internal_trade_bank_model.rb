  ...
  class InternalTradeBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalTradeBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalTradeBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      trade_type_validator = EnumAttributeValidator.new('String', ["platform", "internal", "liquidation", "exit"])
    ...
    def trade_type=(trade_type)
      validator = EnumAttributeValidator.new('String', ["platform", "internal", "liquidation", "exit"])
    ...
    def hash
      [guid, trade_type, customer_guid, quote_guid, symbol, side, state, failure_code, receive_amount, deliver_amount, fee, created_at, updated_at].hash
    end
