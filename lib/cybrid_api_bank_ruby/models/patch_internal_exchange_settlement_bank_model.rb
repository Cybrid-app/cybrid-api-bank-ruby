...
module CybridApiBank
  class PatchInternalExchangeSettlementBankModel
    # ISO8601 datetime the exchange settlement was stored at.
    attr_accessor :stored_at

    # ISO8601 datetime the exchange settlement was approved at.
    attr_accessor :approved_at

    # ISO8601 datetime the exchange settlement was completed at.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalExchangeSettlementBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalExchangeSettlementBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [stored_at, approved_at, completed_at].hash
    end
