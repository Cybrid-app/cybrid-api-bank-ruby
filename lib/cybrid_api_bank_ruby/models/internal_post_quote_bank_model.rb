...
module CybridApiBank
  class InternalPostQuoteBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalPostQuoteBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalPostQuoteBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@symbol.nil? && @symbol.to_s.length > 24
    ...
    def valid?
      ...
      product_type_validator = EnumAttributeValidator.new('String', ["book_transfer", "trading_liquidation", "internal_trading", "internal_bank_rebalance", "internal_wallet_rebalance", "loss_recovery_transfer"])
      ...
      return false if !@customer_guid.nil? && @customer_guid.to_s.length < 32
      return false if !@symbol.nil? && @symbol.to_s.length > 24
    ...
    def product_type=(product_type)
      validator = EnumAttributeValidator.new('String', ["book_transfer", "trading_liquidation", "internal_trading", "internal_bank_rebalance", "internal_wallet_rebalance", "loss_recovery_transfer"])
    ...
    def side=(side)
      validator = EnumAttributeValidator.new('String', ["buy", "sell", "deposit", "withdrawal"])
    ...
    def hash
      [product_type, bank_guid, customer_guid, symbol, asset, side, receive_amount, deliver_amount, environment, source_account_guid, destination_account_guid].hash
    end
