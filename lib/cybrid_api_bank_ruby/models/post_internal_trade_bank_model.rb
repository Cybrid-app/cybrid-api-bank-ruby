...
module CybridApiBank
  class PostInternalTradeBankModel
    # The type of trade.
    ...
    # The identifier for the source account to use for the trade. Required if there are multiple cybrid accounts.
    attr_accessor :source_account_guid

    # The identifier for the destination account to use for the trade. Required if there are multiple cybrid accounts.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalTradeBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalTradeBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      trade_type_validator = EnumAttributeValidator.new('String', ["internal", "liquidation"])
    ...
    def trade_type=(trade_type)
      validator = EnumAttributeValidator.new('String', ["internal", "liquidation"])
    ...
    def hash
      [trade_type, fiat_account_guid, source_account_guid, destination_account_guid, quote_guid, environment].hash
    end
