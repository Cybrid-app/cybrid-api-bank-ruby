...
module CybridApiBank
  class InternalBankBankModel
    ...
    attr_accessor :persona_theme_id

    # The bank's custom return code thresholds.
    attr_accessor :return_code_thresholds
    ...
    def self.attribute_map
      {
        ...
        :'return_code_thresholds' => :'return_code_thresholds',
    ...
    def self.openapi_types
      {
        ...
        :'supported_payout_symbols' => :'Array<BankSupportedPayoutSymbolsInnerBankModel>',
        ...
        :'return_code_thresholds' => :'Hash<String, Integer>',
        :'created_at' => :'Time',
        :'updated_at' => :'Time'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalBankBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalBankBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'return_code_thresholds')
        if (value = attributes[:'return_code_thresholds']).is_a?(Hash)
          self.return_code_thresholds = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @organization_guid.to_s.length > 32
        invalid_properties.push('invalid value for "organization_guid", the character length must be smaller than or equal to 32.')
      end

      if @organization_guid.to_s.length < 32
        invalid_properties.push('invalid value for "organization_guid", the character length must be great than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if @organization_guid.to_s.length > 32
      return false if @organization_guid.to_s.length < 32
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          return_code_thresholds == o.return_code_thresholds &&
    ...
    def hash
      [guid, organization_guid, name, type, supported_trading_symbols, supported_payout_symbols, supported_fiat_account_assets, supported_country_codes, features, cors_allowed_origins, persona_theme_id, return_code_thresholds, created_at, updated_at].hash
    end
