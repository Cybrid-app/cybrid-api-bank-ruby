...
module CybridApiBank
  class PatchInternalTransferScreeningBankModel
    # ISO8601 datetime the transfer screening was started at.
    attr_accessor :started_at

    # ISO8601 datetime the transfer screening was reviewing at.
    attr_accessor :reviewing_at

    # ISO8601 datetime the transfer screening was completed at.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalTransferScreeningBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalTransferScreeningBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [started_at, reviewing_at, completed_at, outcome, failure_code, hold_requested].hash
    end
