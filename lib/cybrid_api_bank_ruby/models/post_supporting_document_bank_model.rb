  ...
  class PostSupportingDocumentBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostSupportingDocumentBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostSupportingDocumentBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'file_guids')
        if (value = attributes[:'file_guids']).is_a?(Array)
          self.file_guids = value
        end
      end
    ...
    def list_invalid_properties
      ...
      if @file_guids.nil?
        invalid_properties.push('invalid value for "file_guids", file_guids cannot be nil.')
      end

      if @file_guids.length > 3
        invalid_properties.push('invalid value for "file_guids", number of items must be less than or equal to 3.')
      end

      if @file_guids.length < 1
        invalid_properties.push('invalid value for "file_guids", number of items must be greater than or equal to 1.')
      end

      if !@document_reference_number.nil? && @document_reference_number.to_s.length < 1
        invalid_properties.push('invalid value for "document_reference_number", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @file_guids.nil?
      return false if @file_guids.length > 3
      return false if @file_guids.length < 1
      return false if !@document_reference_number.nil? && @document_reference_number.to_s.length < 1
      true
    end
    ...
    # @param [Object] file_guids Value to be assigned
    def file_guids=(file_guids)
      if file_guids.nil?
        fail ArgumentError, 'file_guids cannot be nil'
      end

      if file_guids.length > 3
        fail ArgumentError, 'invalid value for "file_guids", number of items must be less than or equal to 3.'
      end

      if file_guids.length < 1
        fail ArgumentError, 'invalid value for "file_guids", number of items must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] document_reference_number Value to be assigned
    def document_reference_number=(document_reference_number)
      if !document_reference_number.nil? && document_reference_number.to_s.length < 1
        fail ArgumentError, 'invalid value for "document_reference_number", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [type, file_guids, document_reference_number].hash
    end
