...
module CybridApiBank
  class PlatformFileBankModel
    ...
    # The file type; one of drivers_license_front, drivers_license_back, drivers_license, passport, passport_card, visa, identification_card, residence_card, work_permit, indigenous_id_document, selfie, selfie_video, selfie_left, selfie_right, utility_bill, proof_of_address, bank_statement, property_tax, tax_document, ein_letter, incorporation_certificate, invoice, purchase_order, delivery_slip, contract, bill_of_lading, others, persona_inquiry_report, or persona_inquiry_export.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PlatformFileBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PlatformFileBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      return false if !@organization_guid.nil? && @organization_guid.to_s.length > 32
      return false if !@organization_guid.nil? && @organization_guid.to_s.length < 32
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "completed", "failed"])
    ...
    def hash
      [guid, organization_guid, bank_guid, customer_guid, type, content_type, filename, completed_at, failed_at, state, failure_code, upload_url, upload_expires_at, download_url, download_expires_at].hash
    end
