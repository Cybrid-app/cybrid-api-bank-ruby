...
module CybridApiBank
  class IdentityVerificationBusinessAssociateBankModel
    ...
    attr_accessor :persona_inquiry_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::IdentityVerificationBusinessAssociateBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::IdentityVerificationBusinessAssociateBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'persona_inquiry_id')
        self.persona_inquiry_id = attributes[:'persona_inquiry_id']
      end

      if attributes.key?(:'persona_state')
        self.persona_state = attributes[:'persona_state']
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          persona_inquiry_id == o.persona_inquiry_id &&
          persona_state == o.persona_state &&
    ...
    def hash
      [persona_inquiry_id, persona_state, customer_guid, role, email_address, state].hash
    end
