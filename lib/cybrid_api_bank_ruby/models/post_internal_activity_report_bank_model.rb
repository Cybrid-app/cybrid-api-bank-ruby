...
module CybridApiBank
  class PostInternalActivityReportBankModel
    # The activity report's type
    attr_accessor :type

    # The environment for the customer.
    attr_accessor :environment

    # The identifier of the customer for the report.
    attr_accessor :customer_guid

    # The asset code to report on.
    attr_accessor :asset

    # Inclusive ISO8601 datetime start time of the period to report on.
    attr_accessor :start_time

    # Inclusive ISO8601 datetime end time of the period to report on.
    attr_accessor :end_time

    # The list of guids to exclude from the report.
    attr_accessor :exclude_guids
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalActivityReportBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalActivityReportBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'exclude_guids')
        if (value = attributes[:'exclude_guids']).is_a?(Array)
          self.exclude_guids = value
        end
      end
    end
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["book", "crypto", "funding", "trade_primary_asset", "trade_counter_asset"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["book", "crypto", "funding", "trade_primary_asset", "trade_counter_asset"])
    ...
    def hash
      [type, environment, customer_guid, asset, start_time, end_time, exclude_guids].hash
    end
