...
module CybridApiBank
  ...
  class PatchInternalAccountAssociationBankModel
    # The side of the account association.
    attr_accessor :side

    # The executed amount in base units of the currency.
    attr_accessor :executed_amount

    # The quoted amount in base units of the currency.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalAccountAssociationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalAccountAssociationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      side_validator = EnumAttributeValidator.new('String', ["source", "destination"])
    ...
    def hash
      [side, executed_amount, quoted_amount].hash
    end
