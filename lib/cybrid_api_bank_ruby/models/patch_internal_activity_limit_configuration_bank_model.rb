...
module CybridApiBank
  ...
  class PatchInternalActivityLimitConfigurationBankModel
    # The limits for the configuration.
    ...
    def self.openapi_types
      {
        :'limits' => :'Array<ParamInternalActivityLimitBankModel>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalActivityLimitConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalActivityLimitConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @limits.length > 50
        invalid_properties.push('invalid value for "limits", number of items must be less than or equal to 50.')
      end
    ...
    def valid?
      ...
      return false if @limits.length > 50
      true
    end
    ...
    def limits=(limits)
      ...
      if limits.length > 50
        fail ArgumentError, 'invalid value for "limits", number of items must be less than or equal to 50.'
      end
