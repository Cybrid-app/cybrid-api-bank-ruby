...
module CybridApiBank
  class InternalPostFeeConfigurationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalPostFeeConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalPostFeeConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@counter_asset.nil? && @counter_asset.to_s.length > 16
        invalid_properties.push('invalid value for "counter_asset", the character length must be smaller than or equal to 16.')
      end

      if !@counter_asset.nil? && @counter_asset.to_s.length < 1
        invalid_properties.push('invalid value for "counter_asset", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      product_type_validator = EnumAttributeValidator.new('String', ["trading", "funding", "crypto_transfer", "invoice", "lightning_transfer", "payout"])
      ...
      return false if !@counter_asset.nil? && @counter_asset.to_s.length > 16
      return false if !@counter_asset.nil? && @counter_asset.to_s.length < 1
    ...
    def product_type=(product_type)
      validator = EnumAttributeValidator.new('String', ["trading", "funding", "crypto_transfer", "invoice", "lightning_transfer", "payout"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] counter_asset Value to be assigned
    def counter_asset=(counter_asset)
      if !counter_asset.nil? && counter_asset.to_s.length > 16
        fail ArgumentError, 'invalid value for "counter_asset", the character length must be smaller than or equal to 16.'
      end

      if !counter_asset.nil? && counter_asset.to_s.length < 1
        fail ArgumentError, 'invalid value for "counter_asset", the character length must be great than or equal to 1.'
      end
    ...
    def hash
      [type, product_type, asset, counter_asset, fees, bank_guid].hash
    end
