...
module CybridApiBank
  ...
  class IdentityVerificationOptionsBankModel
    # Whether the tax ID was enforced during identity verification.
    attr_accessor :require_tax_id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::IdentityVerificationOptionsBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::IdentityVerificationOptionsBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'require_tax_id')
        self.require_tax_id = attributes[:'require_tax_id']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          require_tax_id == o.require_tax_id
    end
