...
module CybridApiBank
  ...
  class PostInternalTransactionMonitorBankModel
    # Type of transaction_monitor to create.
    attr_accessor :type

    # The account identifier. Required when type is account.
    attr_accessor :account_guid

    # The direction to monitor for transactions Optional when type is account.
    attr_accessor :direction

    # The interval in seconds to poll for transactions. Required when type is account.
    attr_accessor :polling_interval

    # The date to start monitoring transactions. Required when type is account.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalTransactionMonitorBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalTransactionMonitorBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [type, account_guid, direction, polling_interval, start_date].hash
    end
