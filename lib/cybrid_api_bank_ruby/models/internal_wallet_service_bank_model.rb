...
module CybridApiBank
  class InternalWalletServiceBankModel
    # Auto-generated unique identifier for the wallet service.
    attr_accessor :guid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalWalletServiceBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalWalletServiceBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      type_validator = EnumAttributeValidator.new('String', ["fireblocks", "circle", "strike", "bitgo", "falconx", "aquanow", "bitso"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["fireblocks", "circle", "strike", "bitgo", "falconx", "aquanow", "bitso"])
    ...
    def hash
      [guid, type, name, environment, created_at, updated_at, state].hash
    end
