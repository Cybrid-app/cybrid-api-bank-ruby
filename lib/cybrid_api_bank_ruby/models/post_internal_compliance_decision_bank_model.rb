...
module CybridApiBank
  class PostInternalComplianceDecisionBankModel
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["business_watchlists", "business_verification", "business_tax_id_verification", "business_attested", "person_attested", "person_tax_id_attested", "person_watchlists", "person_verification", "person_authentication", "person_gov_id_verification", "person_tax_id_verification", "external_bank_account_verification", "external_bank_account_attested"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] identity_verification_guid Value to be assigned
    def identity_verification_guid=(identity_verification_guid)
      if !identity_verification_guid.nil? && identity_verification_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "identity_verification_guid", the character length must be smaller than or equal to 32.'
      end

      if !identity_verification_guid.nil? && identity_verification_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "identity_verification_guid", the character length must be great than or equal to 32.'
      end
