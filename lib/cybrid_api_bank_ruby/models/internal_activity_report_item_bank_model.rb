...
module CybridApiBank
  class InternalActivityReportItemBankModel
    # The item's type
    attr_accessor :type
    ...
    # The pending amount in base units of the asset
    attr_accessor :pending_amount

    # The completed amount in base units of the asset
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalActivityReportItemBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalActivityReportItemBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["deposit", "withdrawal", "buy", "sell"])
    ...
    def hash
      [type, asset, pending_amount, completed_amount].hash
    end
