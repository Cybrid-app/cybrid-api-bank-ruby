  ...
  class PostFeeBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostFeeBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostFeeBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'spread_fee')
        self.spread_fee = attributes[:'spread_fee']
      end
    ...
    def list_invalid_properties
      ...
      if !@spread_fee.nil? && @spread_fee < 1
        invalid_properties.push('invalid value for "spread_fee", must be greater than or equal to 1.')
      end

      if !@fixed_fee.nil? && @fixed_fee < 1
        invalid_properties.push('invalid value for "fixed_fee", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@spread_fee.nil? && @spread_fee < 1
      return false if !@fixed_fee.nil? && @fixed_fee < 1
      true
    end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["spread", "fixed"])
    ...
    # @param [Object] spread_fee Value to be assigned
    def spread_fee=(spread_fee)
      if !spread_fee.nil? && spread_fee < 1
        fail ArgumentError, 'invalid value for "spread_fee", must be greater than or equal to 1.'
      end
    ...
    # Custom attribute writer method with validation
    # @param [Object] fixed_fee Value to be assigned
    def fixed_fee=(fixed_fee)
      if !fixed_fee.nil? && fixed_fee < 1
        fail ArgumentError, 'invalid value for "fixed_fee", must be greater than or equal to 1.'
      end
    ...
    def hash
      [type, spread_fee, fixed_fee].hash
    end
