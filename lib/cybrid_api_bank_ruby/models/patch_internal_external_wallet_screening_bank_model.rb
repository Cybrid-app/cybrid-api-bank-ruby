...
module CybridApiBank
  class PatchInternalExternalWalletScreeningBankModel
    # ISO8601 datetime the external wallet screening was started at.
    attr_accessor :started_at

    # ISO8601 datetime the external wallet screening was decided at.
    attr_accessor :decided_at

    # The outcome of the external wallet screening.
    attr_accessor :outcome

    # The comment to justify the outcome.
    attr_accessor :comment

    # An internal reference to look up the case.
    attr_accessor :reference_id

    # The failure code for failed screenings.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalExternalWalletScreeningBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalExternalWalletScreeningBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      outcome_validator = EnumAttributeValidator.new('String', ["approved", "denied", "failed"])
      ...
      failure_code_validator = EnumAttributeValidator.new('String', ["invalid_address"])
    ...
    def outcome=(outcome)
      validator = EnumAttributeValidator.new('String', ["approved", "denied", "failed"])
    ...
    def failure_code=(failure_code)
      validator = EnumAttributeValidator.new('String', ["invalid_address"])
    ...
    def hash
      [started_at, decided_at, outcome, comment, reference_id, failure_code].hash
    end
