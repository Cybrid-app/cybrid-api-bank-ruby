  ...
  class PostTradeBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostTradeBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostTradeBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      trade_type_validator = EnumAttributeValidator.new('String', ["platform", "exit"])
    ...
    def trade_type=(trade_type)
      validator = EnumAttributeValidator.new('String', ["platform", "exit"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] expected_error Object to be assigned
    def expected_error=(expected_error)
      validator = EnumAttributeValidator.new('String', ["expired_quote", "non_sufficient_funds"])
      unless validator.valid?(expected_error)
        fail ArgumentError, "invalid value for \"expected_error\", must be one of #{validator.allowable_values}."
      end
      @expected_error = expected_error
    end
    ...
    def hash
      [trade_type, quote_guid, fiat_account_guid, expected_error, labels].hash
    end
