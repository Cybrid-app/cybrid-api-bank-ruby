...
module CybridApiBank
  class InternalInternalWalletConfigurationBankModel
    ...
    # The identifier of the internal wallet to use for creating invoices.
    attr_accessor :invoice_wallet_guid

    # The identifier of the internal wallet to use for crypto cold storage.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalInternalWalletConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalInternalWalletConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [guid, type, environment, bank_guid, transfer_wallet_guid, invoice_wallet_guid, storage_wallet_guid, asset, created_at, updated_at].hash
    end
