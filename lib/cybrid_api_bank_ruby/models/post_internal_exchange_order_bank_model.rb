...
module CybridApiBank
  ...
  class PostInternalExchangeOrderBankModel
    ...
    # The trade guids to generate an order for
    ...
    # The source payable account guid to use for the order
    attr_accessor :source_payable_account_guid

    # The source receivable account guid to use for the order
    attr_accessor :source_receivable_account_guid

    # The side to fix Optional when type is fixed.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalExchangeOrderBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalExchangeOrderBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @trade_guids.length > 225
      return false if @trade_guids.length < 1
      side_validator = EnumAttributeValidator.new('String', ["buy", "sell"])
      return false unless side_validator.valid?(@side)
      fixed_amount_validator = EnumAttributeValidator.new('String', ["deliver", "receive"])
      return false unless fixed_amount_validator.valid?(@fixed_amount)
      true
    end
    ...
    def trade_guids=(trade_guids)
      if trade_guids.nil?
        fail ArgumentError, 'trade_guids cannot be nil'
      end
    ...
    def fixed_amount=(fixed_amount)
      validator = EnumAttributeValidator.new('String', ["deliver", "receive"])
      unless validator.valid?(fixed_amount)
        fail ArgumentError, "invalid value for \"fixed_amount\", must be one of #{validator.allowable_values}."
      end
      @fixed_amount = fixed_amount
    end
    ...
    def hash
      [environment, type, trade_guids, side, symbol, deliver_amount, receive_amount, source_payable_account_guid, source_receivable_account_guid, fixed_amount].hash
    end
