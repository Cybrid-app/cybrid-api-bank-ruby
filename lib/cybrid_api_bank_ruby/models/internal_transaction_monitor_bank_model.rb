...
module CybridApiBank
  ...
  class InternalTransactionMonitorBankModel
    ...
    # Custom attribute writer method with validation
    # @param [Object] polling_interval Value to be assigned
    def polling_interval=(polling_interval)
      if polling_interval.nil?
        fail ArgumentError, 'polling_interval cannot be nil'
      end

      if polling_interval < 1
        fail ArgumentError, 'invalid value for "polling_interval", must be greater than or equal to 1.'
      end
    ...
    def hash
      [guid, created_at, updated_at, type, account_guid, directions, polling_interval, start_date].hash
    end
