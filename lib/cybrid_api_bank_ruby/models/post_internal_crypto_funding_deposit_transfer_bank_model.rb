...
module CybridApiBank
  class PostInternalCryptoFundingDepositTransferBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalCryptoFundingDepositTransferBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalCryptoFundingDepositTransferBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @deposit_address_guid.nil?
        invalid_properties.push('invalid value for "deposit_address_guid", deposit_address_guid cannot be nil.')
      end

      if @deposit_address_guid.to_s.length > 32
      ...
      if @txn_hash.nil?
        invalid_properties.push('invalid value for "txn_hash", txn_hash cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @deposit_address_guid.nil?
      return false if @deposit_address_guid.to_s.length > 32
      return false if @deposit_address_guid.to_s.length < 32
      return false if @txn_hash.nil?
    ...
    def deposit_address_guid=(deposit_address_guid)
      if deposit_address_guid.nil?
        fail ArgumentError, 'deposit_address_guid cannot be nil'
      end
    ...
    def hash
      [environment, deposit_address_guid, txn_hash, amount].hash
    end
