...
module CybridApiBank
  class InternalInternalWalletBankModel
    ...
    attr_accessor :platform_balance

    # The amount of funds that are in the account, in base units of the asset, that are available for use on the platform.
    attr_accessor :platform_available

    # The guid of the internal wallet group that this wallet belongs to.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalInternalWalletBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalInternalWalletBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [guid, type, name, asset, account_kind, environment, created_at, updated_at, state, platform_balance, platform_available, internal_wallet_group_guid].hash
    end
