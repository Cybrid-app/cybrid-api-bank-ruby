...
module CybridApiBank
  class PostInternalBankAccountServiceBankModel
    # The account environment.
    attr_accessor :environment

    # The type of bank account service.
    attr_accessor :type

    # The name of the bank account service.
    attr_accessor :name

    # The id of the account at the third-party service.
    attr_accessor :provider_id

    # The unique identifier for the bank account service.
    attr_accessor :guid
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalBankAccountServiceBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalBankAccountServiceBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["modern_treasury", "circle", "strike", "falconx", "bitso", "aquanow"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["modern_treasury", "circle", "strike", "falconx", "bitso", "aquanow"])
    ...
    def hash
      [environment, type, name, provider_id, guid].hash
    end
