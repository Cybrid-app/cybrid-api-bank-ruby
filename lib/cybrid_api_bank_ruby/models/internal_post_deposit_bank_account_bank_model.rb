...
module CybridApiBank
  class InternalPostDepositBankAccountBankModel
    # The account type.
    attr_accessor :type
    ...
    # Optional provider_id to use for the account.  Can only be set in Sandbox mode. It is automatically set in Production. Strictly for testing purposes.
    attr_accessor :provider_id

    # Optional unique_memo_id to use for the account.  Can only be set in Sandbox mode. It is automatically set in Production. Strictly for testing purposes.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalPostDepositBankAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalPostDepositBankAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [type, account_guid, customer_guid, parent_deposit_bank_account_guid, provider_id, unique_memo_id, labels].hash
    end
