...
module CybridApiBank
  class PostInternalInternalBankAccountRoutingDetailBankModel
    attr_accessor :route_type

    # The provider supplied identifier for the route
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalInternalBankAccountRoutingDetailBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalInternalBankAccountRoutingDetailBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      route_type_validator = EnumAttributeValidator.new('String', ["circle_recipient_address", "strike_payment_method", "modern_treasury_external_account", "falconx_whitelisted_address", "aquanow_address_id"])
    ...
    def route_type=(route_type)
      validator = EnumAttributeValidator.new('String', ["circle_recipient_address", "strike_payment_method", "modern_treasury_external_account", "falconx_whitelisted_address", "aquanow_address_id"])
    ...
    def hash
      [route_type, provider_id, bank_account_service_guid, beneficiary_memo].hash
    end
