...
module CybridApiBank
  class PostInternalTransferRailConfigurationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalTransferRailConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalTransferRailConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'rail')
        self.rail = attributes[:'rail']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @hold_duration.nil?
        invalid_properties.push('invalid value for "hold_duration", hold_duration cannot be nil.')
      end

      if @rail.nil?
        invalid_properties.push('invalid value for "rail", rail cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if !@bank_guid.nil? && @bank_guid.to_s.length < 32
      return false if @hold_duration.nil?
      return false if @rail.nil?
      rail_validator = EnumAttributeValidator.new('String', ["ach", "eft", "rtp", "wire", "sandbox", "blockchain"])
      return false unless rail_validator.valid?(@rail)
      true
    end
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] rail Object to be assigned
    def rail=(rail)
      validator = EnumAttributeValidator.new('String', ["ach", "eft", "rtp", "wire", "sandbox", "blockchain"])
      unless validator.valid?(rail)
        fail ArgumentError, "invalid value for \"rail\", must be one of #{validator.allowable_values}."
      end
      @rail = rail
    end
    ...
    def hash
      [type, environment, bank_guid, hold_duration, rail].hash
    end
