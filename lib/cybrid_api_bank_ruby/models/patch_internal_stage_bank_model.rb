...
module CybridApiBank
  ...
  class PatchInternalStageBankModel
    # The state of the stage.
    attr_accessor :state

    # The failure code for the stage, if applicable.
    attr_accessor :failure_code

    # The timestamp when the stage failed, if applicable.
    attr_accessor :completed_at

    # The timestamp when the stage failed, if applicable.
    attr_accessor :failed_at

    # The accounts associated with the stage.
    attr_accessor :accounts

    # The custom fees associated with the stage
    attr_accessor :fees
    ...
    def self.openapi_types
      {
        ...
        :'completed_at' => :'Time',
        :'failed_at' => :'Time',
        :'accounts' => :'Array<PatchInternalAccountAssociationBankModel>',
        :'fees' => :'Array<PatchInternalFeeAssociationBankModel>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalStageBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalStageBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["storing", "planning", "planned", "executing", "completed", "failed"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "planning", "planned", "executing", "completed", "failed"])
    ...
    def hash
      [state, failure_code, completed_at, failed_at, accounts, fees].hash
    end
