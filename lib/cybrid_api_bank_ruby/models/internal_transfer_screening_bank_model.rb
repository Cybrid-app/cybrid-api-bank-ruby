...
module CybridApiBank
  class InternalTransferScreeningBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalTransferScreeningBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalTransferScreeningBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'hold_requested')
        self.hold_requested = attributes[:'hold_requested']
      end
    ...
    def valid?
      ...
      return false if !@transfer_guid.nil? && @transfer_guid.to_s.length < 32
      state_validator = EnumAttributeValidator.new('String', ["storing", "waiting", "reviewing", "completed"])
      return false unless state_validator.valid?(@state)
      outcome_validator = EnumAttributeValidator.new('String', ["approved", "denied", "failed", "dismissed"])
      return false unless outcome_validator.valid?(@outcome)
      failure_code_validator = EnumAttributeValidator.new('String', ["invalid_operation"])
      return false unless failure_code_validator.valid?(@failure_code)
      hold_requested_validator = EnumAttributeValidator.new('String', ["none", "administrative", "non_administrative"])
      return false unless hold_requested_validator.valid?(@hold_requested)
      true
    end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "waiting", "reviewing", "completed"])
    ...
    def outcome=(outcome)
      validator = EnumAttributeValidator.new('String', ["approved", "denied", "failed", "dismissed"])
    ...
    def failure_code=(failure_code)
      validator = EnumAttributeValidator.new('String', ["invalid_operation"])
    ...
    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] hold_requested Object to be assigned
    def hold_requested=(hold_requested)
      validator = EnumAttributeValidator.new('String', ["none", "administrative", "non_administrative"])
      unless validator.valid?(hold_requested)
        fail ArgumentError, "invalid value for \"hold_requested\", must be one of #{validator.allowable_values}."
      end
      @hold_requested = hold_requested
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          hold_requested == o.hold_requested &&
          features == o.features
    end
    ...
    def hash
      [guid, created_at, updated_at, transfer_guid, state, outcome, failure_code, hold_requested, features].hash
    end
