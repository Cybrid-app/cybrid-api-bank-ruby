...
module CybridApiBank
  class PatchInternalCustomerBankModel
    # ISO8601 datetime the customer stored at.
    attr_accessor :stored_at

    # Flag indicating if the customer is frozen or not.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PatchInternalCustomerBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PatchInternalCustomerBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      state_validator = EnumAttributeValidator.new('String', ["unverified"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["unverified"])
