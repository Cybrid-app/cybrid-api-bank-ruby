  ...
  class IdentityVerificationWithDetailsPiiBankModel
    ...
    def self.openapi_types
      {
        ...
        :'registered_address' => :'IdentityVerificationWithDetailsPiiRegisteredAddressBankModel',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::IdentityVerificationWithDetailsPiiBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::IdentityVerificationWithDetailsPiiBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [name, aliases, address, date_of_birth, phone_number, email_address, occupation, citizenship, website, nature_of_business, registered_address, business_industry, business_funds_source, business_funds_destination, business_type, business_incorporation_date, identification_numbers].hash
    end
