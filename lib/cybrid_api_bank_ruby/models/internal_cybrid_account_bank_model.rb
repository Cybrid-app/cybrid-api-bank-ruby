...
module CybridApiBank
  class InternalCybridAccountBankModel
    # The account type.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalCybridAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalCybridAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@asset.nil? && @asset.to_s.length > 128
        invalid_properties.push('invalid value for "asset", the character length must be smaller than or equal to 128.')
      end
    ...
    def valid?
      type_validator = EnumAttributeValidator.new('String', ["fee", "gas", "position"])
      return false unless type_validator.valid?(@type)
      return false if !@guid.nil? && @guid.to_s.length > 32
      return false if !@guid.nil? && @guid.to_s.length < 32
      return false if !@asset.nil? && @asset.to_s.length > 128
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["fee", "gas", "position"])
    ...
    def asset=(asset)
      if !asset.nil? && asset.to_s.length > 128
        fail ArgumentError, 'invalid value for "asset", the character length must be smaller than or equal to 128.'
      end
    ...
    def hash
      [type, guid, created_at, updated_at, asset, name, environment, state].hash
    end
