...
module CybridApiBank
  class InternalCountryCodeConfigurationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalCountryCodeConfigurationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalCountryCodeConfigurationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@code.nil? && @code.to_s.length < 2
        invalid_properties.push('invalid value for "code", the character length must be great than or equal to 2.')
      end
    ...
    def code=(code)
      ...
      if !code.nil? && code.to_s.length < 2
        fail ArgumentError, 'invalid value for "code", the character length must be great than or equal to 2.'
      end
    ...
    def hash
      [guid, type, environment, bank_guid, jurisdiction_enabled, business_customers_enabled, individual_customers_enabled, business_counterparties_enabled, individual_counterparties_enabled, code, created_at, updated_at].hash
    end
