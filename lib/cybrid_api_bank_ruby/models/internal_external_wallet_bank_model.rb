...
module CybridApiBank
  class InternalExternalWalletBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalExternalWalletBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalExternalWalletBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@address.nil? && @address.to_s.length > 1024
        invalid_properties.push('invalid value for "address", the character length must be smaller than or equal to 1024.')
      end
    ...
    def valid?
      ...
      account_kind_validator = EnumAttributeValidator.new('String', ["fireblocks_external_wallet", "circle_wire_external_wallet"])
      ...
      return false if !@customer_guid.nil? && @customer_guid.to_s.length < 32
      return false if !@address.nil? && @address.to_s.length > 1024
      return false if !@tag.nil? && @tag.to_s.length > 512
      state_validator = EnumAttributeValidator.new('String', ["storing", "pending", "failed", "completed", "deleting", "deleted"])
    ...
    def account_kind=(account_kind)
      validator = EnumAttributeValidator.new('String', ["fireblocks_external_wallet", "circle_wire_external_wallet"])
    ...
    def address=(address)
      if !address.nil? && address.to_s.length > 1024
        fail ArgumentError, 'invalid value for "address", the character length must be smaller than or equal to 1024.'
      end
    ...
    def tag=(tag)
      if !tag.nil? && tag.to_s.length > 512
        fail ArgumentError, 'invalid value for "tag", the character length must be smaller than or equal to 512.'
      end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "pending", "failed", "completed", "deleting", "deleted"])
    ...
    def hash
      [guid, name, asset, account_kind, environment, exchange_guid, customer_guid, address, tag, created_at, updated_at, state, failure_code].hash
    end
