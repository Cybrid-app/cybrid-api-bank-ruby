  ...
  class InternalQuoteBankModel
    ...
    def self.openapi_types
      {
        ...
        :'entries' => :'Array<QuoteEntryBankModel>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalQuoteBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalQuoteBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@guid.nil? && @guid.to_s.length < 32
      product_type_validator = EnumAttributeValidator.new('String', ["trading", "trading_exit", "trading_liquidation", "funding", "book_transfer", "crypto_transfer", "internal_trading", "internal_bank_rebalance", "internal_wallet_rebalance", "loss_recovery_transfer", "inter_account", "lightning_transfer"])
    ...
    def product_type=(product_type)
      validator = EnumAttributeValidator.new('String', ["trading", "trading_exit", "trading_liquidation", "funding", "book_transfer", "crypto_transfer", "internal_trading", "internal_bank_rebalance", "internal_wallet_rebalance", "loss_recovery_transfer", "inter_account", "lightning_transfer"])
    ...
    def hash
      [guid, product_type, customer_guid, symbol, side, receive_amount, deliver_amount, fee, created_at, updated_at, issued_at, expires_at, asset, network_fee, network_fee_asset, bank_guid, trade_guid, transfer_guid, entries].hash
    end
