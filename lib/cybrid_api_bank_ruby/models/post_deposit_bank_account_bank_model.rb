  ...
  class PostDepositBankAccountBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostDepositBankAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostDepositBankAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'parent_deposit_bank_account_guid')
        self.parent_deposit_bank_account_guid = attributes[:'parent_deposit_bank_account_guid']
      end
    ...
    def list_invalid_properties
      ...
      if !@parent_deposit_bank_account_guid.nil? && @parent_deposit_bank_account_guid.to_s.length > 32
        invalid_properties.push('invalid value for "parent_deposit_bank_account_guid", the character length must be smaller than or equal to 32.')
      end
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["main", "sub_account"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] parent_deposit_bank_account_guid Value to be assigned
    def parent_deposit_bank_account_guid=(parent_deposit_bank_account_guid)
      if !parent_deposit_bank_account_guid.nil? && parent_deposit_bank_account_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "parent_deposit_bank_account_guid", the character length must be smaller than or equal to 32.'
      end

      if !parent_deposit_bank_account_guid.nil? && parent_deposit_bank_account_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "parent_deposit_bank_account_guid", the character length must be great than or equal to 32.'
      end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          parent_deposit_bank_account_guid == o.parent_deposit_bank_account_guid &&
          labels == o.labels
    end
    ...
    def hash
      [name, type, account_guid, customer_guid, parent_deposit_bank_account_guid, labels].hash
    end
