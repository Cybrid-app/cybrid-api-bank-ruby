...
module CybridApiBank
  ...
  class PostInternalReconciliationBankModel
    # The category of the reconciliation.
    attr_accessor :category

    # The confidence of the reconciliation.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalReconciliationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalReconciliationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'transfer_guid')
        self.transfer_guid = attributes[:'transfer_guid']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @transfer_guid.nil?
        invalid_properties.push('invalid value for "transfer_guid", transfer_guid cannot be nil.')
      end

      if @transfer_guid.to_s.length > 32
        invalid_properties.push('invalid value for "transfer_guid", the character length must be smaller than or equal to 32.')
      end

      if @transfer_guid.to_s.length < 32
        invalid_properties.push('invalid value for "transfer_guid", the character length must be great than or equal to 32.')
      end
    ...
    def valid?
      ...
      category_validator = EnumAttributeValidator.new('String', ["transfer", "return"])
      ...
      return false if @transfer_guid.nil?
      return false if @transfer_guid.to_s.length > 32
      return false if @transfer_guid.to_s.length < 32
      true
    end
    ...
    def category=(category)
      validator = EnumAttributeValidator.new('String', ["transfer", "return"])
    ...
    # @param [Object] transfer_guid Value to be assigned
    def transfer_guid=(transfer_guid)
      if transfer_guid.nil?
        fail ArgumentError, 'transfer_guid cannot be nil'
      end

      if transfer_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "transfer_guid", the character length must be smaller than or equal to 32.'
      end

      if transfer_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "transfer_guid", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [category, confidence, direction, transaction_id, transfer_guid].hash
    end
