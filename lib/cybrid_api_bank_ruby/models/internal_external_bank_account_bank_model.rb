...
module CybridApiBank
  class InternalExternalBankAccountBankModel
    ...
    def self.openapi_types
      {
        ...
        :'pii' => :'Array<InternalExternalBankAccountPiiInnerBankModel>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalExternalBankAccountBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalExternalBankAccountBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      state_validator = EnumAttributeValidator.new('String', ["storing", "completed", "failed", "refresh_required", "unverified", "deleting", "deleted"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "completed", "failed", "refresh_required", "unverified", "deleting", "deleted"])
    ...
    def hash
      [guid, name, asset, account_kind, environment, bank_guid, customer_guid, created_at, updated_at, plaid_institution_id, plaid_account_mask, plaid_account_name, state, failure_code, balances, pii].hash
    end
