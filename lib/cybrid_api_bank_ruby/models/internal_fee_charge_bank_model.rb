...
module CybridApiBank
  ...
  class InternalFeeChargeBankModel
    ...
    attr_accessor :exchange_order_guid
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["platform_assignment"])
    ...
    def source_account_guid=(source_account_guid)
      if source_account_guid.nil?
        fail ArgumentError, 'source_account_guid cannot be nil'
      end
    ...
    def destination_account_guid=(destination_account_guid)
      if destination_account_guid.nil?
        fail ArgumentError, 'destination_account_guid cannot be nil'
      end
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "pending", "completed", "failed"])
    ...
    def failure_code=(failure_code)
      validator = EnumAttributeValidator.new('String', ["non_sufficient_funds"])
    ...
    # Custom attribute writer method with validation
    # @param [Object] exchange_order_guid Value to be assigned
    def exchange_order_guid=(exchange_order_guid)
      if !exchange_order_guid.nil? && exchange_order_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "exchange_order_guid", the character length must be smaller than or equal to 32.'
      end

      if !exchange_order_guid.nil? && exchange_order_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "exchange_order_guid", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [environment, guid, type, created_at, updated_at, asset, amount, source_account_guid, destination_account_guid, state, failure_code, exchange_order_guid].hash
    end
