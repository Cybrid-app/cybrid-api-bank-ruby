...
module CybridApiBank
  class InternalExternalWalletScreeningBankModel
    # The identifier of the external wallet screening.
    attr_accessor :guid
    ...
    # The identifier of the external wallet that was screened.
    attr_accessor :external_wallet_guid

    # The state of the external wallet screening.
    attr_accessor :state

    # The outcome of the external wallet screening.
    attr_accessor :outcome

    # The failure code of the external wallet screening.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalExternalWalletScreeningBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalExternalWalletScreeningBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@external_wallet_guid.nil? && @external_wallet_guid.to_s.length < 32
      state_validator = EnumAttributeValidator.new('String', ["storing", "waiting", "completed"])
    ...
    def state=(state)
      validator = EnumAttributeValidator.new('String', ["storing", "waiting", "completed"])
    ...
    def hash
      [guid, created_at, updated_at, external_wallet_guid, state, outcome, failure_code].hash
    end
