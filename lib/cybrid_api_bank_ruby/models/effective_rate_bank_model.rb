  ...
  class EffectiveRateBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::EffectiveRateBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::EffectiveRateBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'source_asset')
        self.source_asset = attributes[:'source_asset']
      else
        self.source_asset = nil
      end

      if attributes.key?(:'destination_asset')
        self.destination_asset = attributes[:'destination_asset']
      else
        self.destination_asset = nil
      end
    ...
    def list_invalid_properties
      ...
      if @source_asset.nil?
        invalid_properties.push('invalid value for "source_asset", source_asset cannot be nil.')
      end

      if @destination_asset.nil?
        invalid_properties.push('invalid value for "destination_asset", destination_asset cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @source_asset.nil?
      return false if @destination_asset.nil?
      true
    end
    ...
    def hash
      [source_asset, destination_asset, quoted, executed].hash
    end
