...
module CybridApiBank
  ...
  class InternalPersonDetailBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::InternalPersonDetailBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::InternalPersonDetailBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'person_guid')
        self.person_guid = attributes[:'person_guid']
      end
    ...
    def list_invalid_properties
      ...
      if @person_guid.nil?
        invalid_properties.push('invalid value for "person_guid", person_guid cannot be nil.')
      end

      if @person_guid.to_s.length > 32
        invalid_properties.push('invalid value for "person_guid", the character length must be smaller than or equal to 32.')
      end

      if @person_guid.to_s.length < 32
        invalid_properties.push('invalid value for "person_guid", the character length must be great than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if @person_guid.nil?
      return false if @person_guid.to_s.length > 32
      return false if @person_guid.to_s.length < 32
    ...
    # Custom attribute writer method with validation
    # @param [Object] person_guid Value to be assigned
    def person_guid=(person_guid)
      if person_guid.nil?
        fail ArgumentError, 'person_guid cannot be nil'
      end

      if person_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "person_guid", the character length must be smaller than or equal to 32.'
      end

      if person_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "person_guid", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [guid, created_at, updated_at, person_guid, revision, state].hash
    end
