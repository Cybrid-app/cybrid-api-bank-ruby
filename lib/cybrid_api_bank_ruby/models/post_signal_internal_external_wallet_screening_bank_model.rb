...
module CybridApiBank
  class PostSignalInternalExternalWalletScreeningBankModel
    # An internal reference to look up the case.
    attr_accessor :reference_id

    # The comment to justify the manual decision.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostSignalInternalExternalWalletScreeningBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostSignalInternalExternalWalletScreeningBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @reference_id.to_s.length > 1024
    ...
    def valid?
      ...
      return false if @reference_id.to_s.length > 1024
      return false if @reference_id.to_s.length < 1
      return false if @comment.nil?
      return false if @comment.to_s.length > 4096
      return false if @comment.to_s.length < 1
      return false if @outcome.nil?
      outcome_validator = EnumAttributeValidator.new('String', ["approved", "denied"])
    ...
    def reference_id=(reference_id)
      ...
      if reference_id.to_s.length > 1024
        fail ArgumentError, 'invalid value for "reference_id", the character length must be smaller than or equal to 1024.'
      end
    ...
    def comment=(comment)
      ...
      if comment.to_s.length > 4096
        fail ArgumentError, 'invalid value for "comment", the character length must be smaller than or equal to 4096.'
      end
    ...
    def outcome=(outcome)
      validator = EnumAttributeValidator.new('String', ["approved", "denied"])
    ...
    def hash
      [reference_id, comment, outcome].hash
    end
