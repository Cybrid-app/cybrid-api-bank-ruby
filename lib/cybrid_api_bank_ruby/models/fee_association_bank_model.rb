  ...
  class FeeAssociationBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::FeeAssociationBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::FeeAssociationBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'stage_guid')
        self.stage_guid = attributes[:'stage_guid']
      end
    ...
    def list_invalid_properties
      ...
      if !@stage_guid.nil? && @stage_guid.to_s.length > 32
        invalid_properties.push('invalid value for "stage_guid", the character length must be smaller than or equal to 32.')
      end

      if !@stage_guid.nil? && @stage_guid.to_s.length < 32
        invalid_properties.push('invalid value for "stage_guid", the character length must be great than or equal to 32.')
      end
    ...
    def valid?
      ...
      return false if !@stage_guid.nil? && @stage_guid.to_s.length > 32
      return false if !@stage_guid.nil? && @stage_guid.to_s.length < 32
    ...
    # @param [Object] stage_guid Value to be assigned
    def stage_guid=(stage_guid)
      if !stage_guid.nil? && stage_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "stage_guid", the character length must be smaller than or equal to 32.'
      end

      if !stage_guid.nil? && stage_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "stage_guid", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [type, stage_guid, asset, quoted_amount, executed_amount].hash
    end
