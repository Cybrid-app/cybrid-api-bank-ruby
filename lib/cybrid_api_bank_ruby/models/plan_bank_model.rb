  ...
  class PlanBankModel
    ...
    def self.openapi_types
      {
        ...
        :'supporting_documents' => :'Array<SupportingDocumentBankModel>',
        :'labels' => :'Array<String>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PlanBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PlanBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [guid, type, bank_guid, customer_guid, created_at, updated_at, expires_at, state, failure_code, source_account, destination_account, intermediate_accounts, stages, fees, effective_rate, travel_rule_info, purpose_of_transaction, supporting_documents, labels].hash
    end
