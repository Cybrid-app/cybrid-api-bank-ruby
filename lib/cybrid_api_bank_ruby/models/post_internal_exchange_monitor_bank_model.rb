...
module CybridApiBank
  ...
  class PostInternalExchangeMonitorBankModel
    # The exchangee identifier.
    attr_accessor :exchange_guid

    # The cron schedule for polling the exchange.
    attr_accessor :cron_schedule

    # The trading symbols to allow in the settlement; omit if all symbols should be allowed.
    attr_accessor :allowed_symbols

    # The trading symbols to deny in the settlement; omit if no symbols should be denied.
    attr_accessor :denied_symbols

    # The country codes to skip settlements on non-workdays; omit if workdays should not be factored.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::PostInternalExchangeMonitorBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::PostInternalExchangeMonitorBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@workday_countries.nil? && @workday_countries.length > 50
        invalid_properties.push('invalid value for "workday_countries", number of items must be less than or equal to 50.')
      end

      if !@workday_countries.nil? && @workday_countries.length < 1
        invalid_properties.push('invalid value for "workday_countries", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@workday_countries.nil? && @workday_countries.length > 50
      return false if !@workday_countries.nil? && @workday_countries.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] workday_countries Value to be assigned
    def workday_countries=(workday_countries)
      if !workday_countries.nil? && workday_countries.length > 50
        fail ArgumentError, 'invalid value for "workday_countries", number of items must be less than or equal to 50.'
      end

      if !workday_countries.nil? && workday_countries.length < 1
        fail ArgumentError, 'invalid value for "workday_countries", number of items must be greater than or equal to 1.'
      end
    ...
    def hash
      [exchange_guid, cron_schedule, allowed_symbols, denied_symbols, workday_countries].hash
    end
