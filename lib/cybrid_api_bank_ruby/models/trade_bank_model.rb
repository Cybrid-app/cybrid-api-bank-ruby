...
module CybridApiBank
  class TradeBankModel
    ...
    # Custom attribute writer method with validation
    # @param [Object] reference_trade_guid Value to be assigned
    def reference_trade_guid=(reference_trade_guid)
      if !reference_trade_guid.nil? && reference_trade_guid.to_s.length > 32
        fail ArgumentError, 'invalid value for "reference_trade_guid", the character length must be smaller than or equal to 32.'
      end

      if !reference_trade_guid.nil? && reference_trade_guid.to_s.length < 32
        fail ArgumentError, 'invalid value for "reference_trade_guid", the character length must be great than or equal to 32.'
      end
    ...
    def hash
      [guid, trade_type, customer_guid, quote_guid, symbol, side, state, failure_code, receive_amount, deliver_amount, fee, fee_details, reference_trade_guid, created_at, updated_at, labels].hash
    end
