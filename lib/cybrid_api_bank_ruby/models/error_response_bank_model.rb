  ...
  class ErrorResponseBankModel
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CybridApiBank::ErrorResponseBankModel` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CybridApiBank::ErrorResponseBankModel`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'message_code')
        self.message_code = attributes[:'message_code']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @message_code.nil?
        invalid_properties.push('invalid value for "message_code", message_code cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @message_code.nil?
      true
    end
    ...
    def hash
      [status, error_message, message_code].hash
    end
